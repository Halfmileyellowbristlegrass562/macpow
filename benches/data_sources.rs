...
fn bench_ioreport(c: &mut Criterion) {
    ...
    c.bench_function("ioreport_sample", |b| {
        b.iter(|| sampler.sample().unwrap())
    });
}
...
fn bench_smc(c: &mut Criterion) {
    ...
    c.bench_function("smc_temperatures", |b| {
        b.iter(|| smc.read_temperatures())
    });
    c.bench_function("smc_fans", |b| {
        b.iter(|| smc.read_fans())
    });
    c.bench_function("smc_system_power", |b| {
        b.iter(|| smc.read_system_power())
    });
...
fn bench_battery(c: &mut Criterion) {
    c.bench_function("battery", |b| {
        b.iter(|| macpow::battery::read_battery())
    });
}

fn bench_peripherals(c: &mut Criterion) {
    c.bench_function("wifi", |b| {
        b.iter(|| macpow::peripherals::read_wifi_info())
    });
...
fn bench_metrics(c: &mut Criterion) {
    ...
    c.bench_function("memory", |b| {
        b.iter(|| macpow::metrics::read_mem_used_gb())
    });
