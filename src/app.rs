...
fn selected_cpu_core_temps(
    ...
    let p_triplets = sorted_temp_values(temps, |key| key.starts_with("Tp1") || key.starts_with("Tp2"));
    ...
    if fallback.len() >= e_count + p_count && !is_placeholder_temp_bank(&fallback, e_count + p_count)
    {
